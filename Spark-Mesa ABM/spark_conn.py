...
def sparkModel(cfgJson, comparisonFile=None):
    ...
    # I think 15minutes one report is better, it can change 
        nextReportTimeStep = durationSeconds*10.0/200.0
    logger.info("Running.. ")
    ...
    while spark_model.run_model(nextReportTimeStep):
        ...
        #ax2.imshow(spark_model.get_arrival().data,interpolation='none', origin='lower')
        #plt.show()
        ...
        #self_extinguish = len(abm_input[np.where(abm_input <= 0)]) - truck_extinguish
