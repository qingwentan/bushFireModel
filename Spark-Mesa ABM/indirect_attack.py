...
class Walker(Agent):
    ...
    # do not need to modify, but river part can remove or not
    ...
    def biggestfire_move(self):
        ...
        # calculate fire fighter to burning vegetation radio (changed)
        ratio = self.firefighters_tree_ratio(
            self.model.num_firetruck, len(self.model.list_burn_cell(self.model)))
        ...
        for i in range(len(limited_vision_list)):
            ...
            neighbors_list = [x for x in neighbors_list if isinstance(x, burnCell) and x.fire_bar != 0]
            ...
            for neighbor in neighbors_list:
                if neighbor.trees_claimed < ratio:
                    current_intensity_bar = neighbor.fire_bar
                    distance = abs(neighbor.pos[0] ** 2 - self.pos[0] ** 2) + \
                        abs(neighbor.pos[1] ** 2 - self.pos[1] ** 2)
                    if current_intensity_bar >= max_fire_bar and distance <= min_distance:
                        min_distance = distance
                        max_fire_bar = current_intensity_bar
    ...
    def optimized_parallel_fire(self):
        ...
        for neighbor in neighbors_list_fire:
            ...
            if isinstance(neighbor, burnCell) and neighbor.fire_bar != 0 and position_x <= self.truck_max_speed and \
        ...
        for i in range(len(limited_vision_list)):
            ...
            for neighbor in neighbors_list:
                if neighbor.trees_claimed < ratio:
                    ...
                    fire_bar = neighbor.fire_bar
                    if distance <= min_distance and fire_bar <= min_fire_bar:
                        min_fire_bar = fire_bar
                        min_distance = distance
                        closest_neighbor = neighbor
                        fire_intheneighborhood = True
            if fire_intheneighborhood:
    ...
    def indirect_attack(self):
        ...
        if self.model.buffer_x_min <= self.pos[0] <= self.model.buffer_x_max and  \
            ...
            if len(self.model.tree_list_on_buffer) != 0:
                ...
                if tree_present and tree_object.fire_bar == 0:
                    self.extinguished += 1
...
class Fireman(Walker):
    ...
    '''A class specific to a fireman'''
    def __init__(self, model, unique_id, pos,
                 man_strategy, vision, man_max_speed):
        super().__init__(unique_id, model, pos)
        self.unique_id = unique_id
        self.condition = "man"
        self.extinguished = 0
        self.man_strategy = man_strategy
        self.vision = vision
        self.man_max_speed = man_max_speed
        self.life_bar = -5
    ...
    def step(self):
        # set step according to strategy
        if (self.man_strategy == 'Goes to the closest fire'):
            self.closestfire_move()
        elif (self.man_strategy == 'Goes to the biggest fire'):
            self.biggestfire_move()
        elif (self.man_strategy == "Parallel attack"):
            self.parallel_attack()
        elif (self.man_strategy == "Optimized Parallel attack"):
            self.optimized_parallel_fire()
        elif (self.man_strategy == "Optimized closest"):
            self.optimized_closest_fire()
        elif (self.man_strategy == 'Indirect attack'):
            self.indirect_attack()
        elif (self.man_strategy == 'Random'):
            self.random_move()

        if (self.man_strategy != 'Indirect attack'):
            self.extinguish()
...
class Dozer(Walker):
    ...
    '''A class specific to a Dozer'''
    def __init__(self, model, unique_id, pos,
                 Dozer_strategy, vision, Dozer_max_speed):
        super().__init__(unique_id, model, pos)
        self.unique_id = unique_id
        self.condition = "Dozer"
        self.extinguished = 0
        self.Dozer_strategy = Dozer_strategy
        self.vision = vision
        self.Dozer_max_speed = Dozer_max_speed
        self.life_bar = -5
    ...
    def step(self):
        # set step according to strategy
        if (self.Dozer_strategy == 'Goes to the closest fire'):
            self.closestfire_move()
        elif (self.Dozer_strategy == 'Goes to the biggest fire'):
            self.biggestfire_move()
        elif (self.Dozer_strategy == "Parallel attack"):
            self.parallel_attack()
        elif (self.Dozer_strategy == "Optimized Parallel attack"):
            self.optimized_parallel_fire()
        elif (self.Dozer_strategy == "Optimized closest"):
            self.optimized_closest_fire()
        elif (self.Dozer_strategy == 'Indirect attack'):
            self.indirect_attack()
        elif (self.Dozer_strategy == 'Random'):
            self.random_move()

        if (self.Dozer_strategy != 'Indirect attack'):
            self.extinguish()
