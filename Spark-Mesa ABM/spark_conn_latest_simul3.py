...
#Code to do a single simulation and save the result into a file
import numpy as np
...
def sparkModel(cfgJson, comparisonFile=None):
    ...
    resultsFile2 = "Closest_agent_edgefalse_01.csv"
    ...
    printtofile = [["Step","Strategy","Number of Truck","Max Speed","Vision","Placed on Edges","Truck Extinguish","Self Extinguish"]]
    ...
    while spark_model.run_model(nextReportTimeStep):
        ...
        #plt.show()
        printtofile.append([stepcount,truck_strategy,num_firetruck,max_speed,vision,placed_on_edges,truck_extinguish,self_extinguish])
        progressPercent += 2.5
        stepcount+=1
    ...
    # Write results
    if results:
        if resultsFile2 is not None:
            logger.info(f"Writing results to file: {resultsFile2}")
            with open(resultsFile2, "w") as rf:
