...
class ForestFire(Model):
    ...
    def step_intensity(self, new_input):
        ...
        for i in range(len(x_array)):
            ...
            if burn_present:
                if new_input[x_array[i]][y_array[i]] <= 0:
                    burn_object.fire_bar = 0
    ...
    @staticmethod
    def count_self_extinguished_fires(model):
        '''
        Helper method to count self-extinguished fires in a given condition in a given model.
        '''
        return model.self_extinguished
